...
import { buildGuildStructureEntries } from "../workspaceHelpers.js";
import { buildDirectMessageShortcuts } from "../workspaceNavigationHelpers.js";
import { createWorkspaceChannelRenderers } from "../workspaceNavigationRenderers.jsx";
...
export function useWorkspaceNavigationState({
  ...
  currentUserLabel,
  dmMenuPrefs,
  draggedChannelId: _ignoredDraggedChannelId,
  draggedGuildId: _ignoredDraggedGuildId,
  guildMenuPrefs,
  ...
  const directMessageShortcuts = React.useMemo(
    () =>
      buildDirectMessageShortcuts({
        activeSelection,
        dmMenuPrefs,
        workspace
      }),
    [activeSelection, dmMenuPrefs, workspace]
  );
  ...
  function handleCategoryDrop(event, categoryId, categoryChannels = []) {
    ...
    const referenceChannel =
      categoryChannels.filter((channel) => channel.id !== draggedChannelIdRef.current).at(-1) ||
      null;
  ...
  const { renderTextChannelRow, renderVoiceChannel } = createWorkspaceChannelRenderers({
    activeGuild,
    activeSelection,
    canDragChannel,
    channelDropHint,
